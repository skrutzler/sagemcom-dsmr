...
def get_serial_data():
    try:
        with Serial(args.device, 115200, timeout=6.0,
            ...
            return jsonify({'name': "current_consumed_active_power",
                            'value': list(response_as_dict["1-0:1.7.0"].values())[0]['value']},
                           {'name': "current_provided_active_power",
                            'value': list(response_as_dict["1-0:2.7.0"].values())[0]['value']},
                           {'name': "total_consumed_energy",
                            'value': list(response_as_dict["1-0:1.8.0"].values())[0]['value']},
                           {'name': "total_provided_energy",
                            'value': list(response_as_dict["1-0:2.8.0"].values())[0]['value']})
